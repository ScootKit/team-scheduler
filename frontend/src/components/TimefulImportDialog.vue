<template>
  <v-dialog v-model="dialog" max-width="500px" content-class="tw-m-0">
    <v-card>
      <v-card-title>
        <span class="tw-text-xl tw-font-medium">Import WannPassts Event</span>
        <v-spacer />
        <v-btn
          absolute
          @click="dialog = false"
      ...
      <v-card-text class="tw-text-very-dark-gray">
        <p class="tw-mb-4">
          Paste a WannPassts event URL from another instance to import it along
  ...
  methods: {
    ...
    async importEvent() {
      ...
      try {
        ...
        const errorMessages = {
          "invalid-url": "Invalid URL. Please enter a valid WannPassts event URL.",
